...
def setup_arg_parser():
    ...
    parser.add_argument(
        "--mtp",
        action="store_true",
        help="Use MTP (Multi-Token Prediction) heads for speculative decoding. "
        "Requires a model with mtp_num_hidden_layers > 0 (e.g. Qwen3.5).",
    )
    return parser
...
def mtp_speculative_generate_step(
    ...
    *,
    ...
    """
    A generator producing token ids using MTP (Multi-Token Prediction) heads
    for speculative decoding.

    The MTP head is a single full-attention transformer layer that reuses the
    main model's hidden states, embeddings, and lm_head to draft tokens.

    Args:
        prompt (mx.array): The input prompt token ids.
        model (nn.Module): The model (must have MTP head via ``has_mtp``).
        ...
        prompt_progress_callback: Callback for prefill progress.

    Yields:
        Tuple[mx.array, mx.array, bool]: One token, log probabilities, and
          whether the token was generated by the draft (MTP head).
    """
    ...
    # Access the underlying TextModel which has MTP head
    text_model = model
    if hasattr(model, "language_model"):
        text_model = model.language_model

    if prompt_cache is None:
        verify_cache = cache.make_prompt_cache(model)
    else:
        verify_cache = prompt_cache
    ...
    def _model_step(y, n_predict=1, output_intermediates=False):
        nonlocal prev_tokens
        with mx.stream(generation_stream):
            logits = text_model(
                y[None], cache=verify_cache,
            ...
            if logits_processors and n_predict > 1:
                out_y, out_logprobs = [], []
                flat_y = y[: -(n_predict - 1)] if n_predict > 1 else y
                for i in range(n_predict):
                    prev_tokens = (
    ...
    # Pre-allocate MTP cache (reuse by trimming each cycle)
    _mtp_cache = text_model.mtp.make_cache()

    def _draft_generate(y, hidden_state, num_draft):
        ...
        ys = []
        for _ in range(num_draft):
            mtp_h, logits = text_model.mtp_draft(mtp_h, tok[None], _mtp_cache)
            tok = mx.argmax(logits[:, -1:, :], axis=-1).reshape(-1)
            mx.async_eval(tok)
            ys.append(tok)
        return mx.concatenate(ys)
    ...
    with mx.stream(generation_stream):
        ...
        while total_prompt_tokens - prompt_processed_tokens > 1:
            remaining = (total_prompt_tokens - prompt_processed_tokens) - 1
            n_to_process = min(prefill_step_size, remaining)
            text_model(prompt[:n_to_process][None], cache=verify_cache)
            quantize_cache_fn(verify_cache)
        ...
        logits = text_model(prompt[None], cache=verify_cache)
        logits = logits[:, -1:, :]
        quantize_cache_fn(verify_cache)
    ...
    hidden_state = text_model._last_hidden_state[:, -1:, :]
    ...
    try:
        while True:
            ...
            draft_tokens = _draft_generate(y, hidden_state, num_draft)

            if num_draft == 0:
                verify_tok, verify_lp = _model_step(y)
                mx.eval(verify_tok)
                hidden_state = text_model._last_hidden_state[:, -1:, :]
                n = 0
                ntoks += 1
                ...
                tokens, logprobs = _model_step(
                    all_tokens, num_draft + 1,
                    output_intermediates=_supports_intermediates,
                )
                mx.eval(tokens, draft_tokens)
                y_prev_item = y.item()
                ...
                if ntoks < max_tokens:
                    ...
                    hidden_state = text_model._last_hidden_state[:, n : n + 1, :]
                    ...
                    if _supports_intermediates:
                        ...
                        if n < num_draft:
                            ...
                            for c in verify_cache:
                                if c.is_trimmable():
                                    c.trim(num_draft + 1)
                            replay_ids = [y_prev_item] + draft_tokens_list[:n]
                            replay = mx.array([replay_ids], mx.uint32)
                            _model_step(replay, len(replay_ids))
                            hidden_state = text_model._last_hidden_state[:, -1:, :]
                    else:
                        n_trim = num_draft - n
                        if n_trim > 0:
...
def stream_generate(
    ...
    **kwargs,
    ...
    if use_mtp:
        if not getattr(text_model, "has_mtp", False):
            raise ValueError(
                "Model does not have MTP heads. "
                "Use a model with mtp_num_hidden_layers > 0 (e.g. Qwen3.5)."
            )
...
def main():
    ...
    if args.draft_model is not None and args.mtp:
        raise ValueError("Cannot use both --draft-model and --mtp")
